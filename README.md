...
```
TIC-361894940-Analysis/
├── README.md                          # This file
├── requirements.txt                   # Python dependencies
├── LICENSE                            # MIT License
│
├── data/
│   ├── tess_lc_sector_XX.fits         # TESS light curve (FITS format)
│   ├── tic_361894940_photometry.csv   # Extracted photometry
│   └── ztf_verification_data.json     # ZTF cross-reference results
│
├── src/
│   ├── __init__.py
│   ├── tess_analysis.py               # TESS light curve processing
│   ├── bls_periodicity.py             # Box-Least-Squares search
│   ├── aperture_analysis.py           # Contamination checks
│   └── ztf_verification.py            # ZTF cross-validation
│
├── notebooks/
│   ├── 01_tess_extraction.ipynb       # Light curve extraction
│   ├── 02_period_search.ipynb         # BLS periodogram
│   ├── 03_aperture_contamination.ipynb # Spatial analysis
│   ├── 04_ztf_crossmatch.ipynb        # ZTF verification
│   └── 05_results_summary.ipynb       # Final analysis
│
├── results/
│   ├── folded_lightcurve.csv          # Phase-folded data
│   ├── periodogram_comparison.csv     # TESS vs ZTF periods
│   ├── aperture_analysis.csv          # Contamination check
│   └── analysis_summary.txt           # Parameter summary
│
└── docs/
    ├── METHODOLOGY.md                 # Detailed analysis workflow
    ├── LESSONS_LEARNED.md             # Scientific best practices
    └── VERIFICATION_REPORT.md         # Cross-validation details
```
...
```bash
# Clone repository
git clone https://github.com/srivathsanharinarayanan-ctrl/M-Dwarf-Brown-Dwarf-Twin-Binaries.git
cd M-Dwarf-Brown-Dwarf-Twin-Binaries
...
```python
from src.tess_analysis import TESSLightCurve
from src.bls_periodicity import BLSSearch
from src.ztf_verification import ZTFCrossReference

# Load TESS data
lc = TESSLightCurve('data/tess_lc_sector_XX.fits')

# Perform BLS periodicity search
bls = BLSSearch(lc)
periods, snr = bls.search(min_period=0.5, max_period=50)

# Cross-reference with ZTF
ztf = ZTFCrossReference(ra=296.5537, dec=23.6440)
ztf_period, ztf_snr = ztf.verify()

# Compare results
print(f"TESS Period: {bls.best_period:.5f} days")
print(f"ZTF Period: {ztf_period:.3f} days")
```

### Jupyter Notebooks

All analysis steps are documented in interactive notebooks:

```bash
jupyter notebook notebooks/01_tess_extraction.ipynb
jupyter notebook notebooks/02_period_search.ipynb
jupyter notebook notebooks/04_ztf_crossmatch.ipynb
```
...
### Box-Least-Squares (BLS) Periodicity Search

Detects periodic transit-like signals in light curves using the BLS algorithm (Kovács et al. 2002):

```
Frequency resolution: 1/T_obs
Transit duration: parameterized by q (duty cycle)
Signal-to-noise: BLS statistic normalized by red noise
```

### Aperture Contamination Analysis

TESS pixels are large (~21 arcsec). We verified:
- Nearby star positions relative to aperture
- Flux contribution estimates
- High-resolution imaging cross-checks

**Finding:** A 17th magnitude blue star is ~1.5 TESS pixels from target; careful aperture isolation required.

### ZTF Cross-Validation

Independent verification using Zwicky Transient Facility:
- Higher spatial resolution than TESS (1.01"/pixel vs 21"/pixel)
- Separate photometric aperture eliminates blending
- Multi-filter observations (zr, zg, zi) detect consistent periodicity

**Result:** Confirms real variability but reveals different period (28.691 vs 13.92657 days).
